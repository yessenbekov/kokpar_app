...
function playerRow({ user, profile, selectedHorse, teamSide }) {
  const side = teamSide === "red" ? "red" : teamSide === "spectator" ? "spectator" : "blue";
  return {
    user_id: user.id,
...
export const onlineRoomStore = {
  ...
  async join({ code, profile, selectedHorse, settings }) {
    ...
    const activePlayers = currentRoom.players.filter((p) => p.team !== "spectator");
    if (!isSpectator && activePlayers.length >= roomRow.team_size * 2 && !currentRoom.players.some((p) => p.userId === user.id)) {
  ...
  async updatePlayer(roomId, updates) {
    ...
    if (updates.teamSide) {
      row.team_side = updates.teamSide === "red" ? "red" : updates.teamSide === "spectator" ? "spectator" : "blue";
    }
  ...
  async kick(roomId, targetUserId) {
    ...
    if (room.hostUserId !== user.id) throw new Error("Только хост может кикать игроков");
    ...
    if (error) throw queryError(error, "Не удалось кикнуть игрока");
    return fetchRoom(roomId);
  },
